# The Control Tower Block — a droppable process discipline for AI coding agents

<!-- WHAT THIS IS: a self-contained block of workflow rules for a project that already has an
     AGENTS.md/CLAUDE.md. Copy everything between the BEGIN/END markers into your file.
     It assumes multiple AI agent sessions (possibly different models/hosts) work your repo,
     with one human Owner making every design and policy decision.

     PRECEDENCE: your host file wins on DOMAIN (security, architecture, product, language rules).
     This block wins on PROCESS (roles, review, records, commits, diagnosis discipline).
     Where a host rule collides with a process rule here, resolve toward the host file for domain
     questions and toward this block for how agents coordinate — and record the resolution in
     your handoff doc so it stops being a collision. -->
...
- **Planner** — scopes work into units with acceptance conditions ("a unit with no acceptance
  condition is not a unit"); writes plan drafts; never resolves the Owner's forks. Either a
  dedicated Planner seat (the package's skill contract) or the CT absorbing the seat — the
  source project runs the latter; the lineage rule binds either way.
